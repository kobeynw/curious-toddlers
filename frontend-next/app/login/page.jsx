...
export default function LoginPage() {
  ...
  const redirectTarget = user?.isVerified ? '/' : '/verify-email';
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      router.push(data?.isVerified ? '/' : '/verify-email');
    } catch (err) {
    ...
    <div className="max-w-sm mx-auto px-4 py-12">
      <div className="bg-sand-surface rounded-lg shadow p-6">
        ...
        <p className="text-sm text-ink-muted mt-4 text-center">
          ...
          <Link href="/register" className="text-terra hover:text-terra-hover">
            Register
          </Link>
        </p>
      </div>
