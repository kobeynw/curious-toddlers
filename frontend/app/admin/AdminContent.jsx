...
function ActivityFormModal({ open, onClose, activity, allTags, onCreated, onUpdated }) {
  ...
  // Reset the form fields whenever the modal opens or the edited activity
  // changes. Done as an in-render update (tracking previous prop identities)
  // rather than a useEffect, per the React 19 "storing information from
  // previous renders" idiom — Next 16's react-hooks/set-state-in-effect rule
  // blocks the build on synchronous setState inside an effect.
  // https://react.dev/reference/react/useState#storing-information-from-previous-renders
  const [prevOpen, setPrevOpen] = useState(open);
  const [prevActivity, setPrevActivity] = useState(activity);
  if (open !== prevOpen || activity !== prevActivity) {
    setPrevOpen(open);
